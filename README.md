...
让**超长程、无人值守**的编码任务获得：崩溃可恢复、原生心跳看门狗、错误分类重试、失败教训累积。
...
| 失败教训不持久，重跑重复踩坑 | 教训落盘到项目本地 `.zeroshot/lessons.jsonl`，重试/后续运行时自动注入 |
...
```
        ...
        └─────────────────────┬─────────────────────┘
                              │
        ┌─────────────────────▼─────────────────────┐
        │   本地教训库 (.zeroshot/lessons.jsonl)       │   ← 失败教训注入
        └───────────────────────────────────────────┘
```
...
- **`loop/`** — Temporal 持久化 loop 核心（Python）。
  ...
  - `models.py` / `zeroshot_lib.py` / `lessons_lib.py` / `classify.py` — 数据模型与封装。
...
`install.sh` 会：检查依赖 → 装 `temporalio` → **打 zeroshot 补丁** → 把 `dloop`/`zs-learn` 链接到 `~/.local/bin`。
...
1. **注入**：每次跑任务前，从项目本地 `.zeroshot/lessons.jsonl` 读取未解决教训，注入到提示词开头。
2. **捕获**：任务失败/卡死时，把教训写进项目本地 `.zeroshot/lessons.jsonl`。
3. **解决**：任务成功且过验证门时，自动把相关教训标记为已解决（不再注入）。

> 教训按项目隔离——每个项目的失败经验只作用于该项目自己的后续运行。
...
│   ├── lessons_lib.py            # 教训本地存储 (lessons.jsonl)
...
└── docs/
    ...
    └── memory-systems-report.md  # 记忆系统分析（历史文档）
```
...
**Q: 教训没被注入？**
教训从项目本地 `.zeroshot/lessons.jsonl` 读取（未解决项）。确认 `--project-dir` 指向正确项目；
可用 `zs-learn lessons` 查看当前教训。
